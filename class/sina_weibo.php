...
//获取微博返回码对应的错误信息
	...
	function get_access_token($code=''){
		...
		if($code!=''){//重新授权获取access_token
			/*
			 * 新浪坑爹的get、post混合请求
			 * 传的参数直接拼在url后面，但需要用post方式提交
			 */
			...
			$token_url .= '?'.http_build_query($post_data);
			$token_json = curl_post_contents($token_url, '');
			$token_obj = @json_decode($token_json);
			if(!is_object($token_obj)){
				return false;
			}else if(isset($token_obj->error_code) and $token_obj->error_code){
				$this->set_error_info($token_obj->error_code, $token_obj->error);
				return false;
			}
			$access_token = $token_obj->access_token;
			$expires_in = $token_obj->expires_in;
			//因为memcache的expire为秒数时不能超过30天，所以expire使用时间戳
			//set_cache('weibo_access_token', $access_token, $expires_in);
			set_cache('weibo_access_token', $access_token, 86400*30);
			return $access_token;
		}else{//从缓存中读取access_token
			$access_token = get_cache('weibo_access_token');
			if(!$access_token){
				write_log('从缓存中读取access_token失败！','sina_weibo.log');
			}
			return $access_token;
		}
	}
	...
	function upload_text($content){
		...
		if($access_token){
			$upload_url = "https://api.weibo.com/2/statuses/update.json";
			$upload_data = array(
				'access_token'=>$access_token,
				'status'=>$content
			);
			$upload_url .= '?'.http_build_query($upload_data);
			$result_json = curl_post_contents($upload_url, '');
			$result_obj = @json_decode($result_json);
			if(!is_object($result_obj)){
				return false;
			}else if(isset($result_obj->error_code) and $result_obj->error_code){
				$this->set_error_info($result_obj->error_code, $result_obj->error);
				return false;
			}
			return $result_obj;
		}else{
	...
	//上传图片并发布一条微博(图文)
	function upload_news($content,$pic_path){
		$access_token = $this->get_access_token();
		if($access_token){
			...
			$result_json = curl_post_contents($upload_url, $upload_data);
			$result_obj = @json_decode($result_json);
			if(!is_object($result_obj)){
				return false;
			}else if(isset($result_obj->error_code) and $result_obj->error_code){
